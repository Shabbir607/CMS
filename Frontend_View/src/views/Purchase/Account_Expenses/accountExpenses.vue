...
<template>
        ...
        <div class="headerinfo">
            ...
            <div class="buttn">
                <a type="button" class="btn1" @:click="addRecordBtn">
                    ...
                    <span class="fw-bold">Add New Expenses</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1"  id="addNewCategory"
        ...
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasRightLabel"> Expenses / Add</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div class="col-md-12">
                <label class="form-label">Expenses Name</label>
                <input type="text" v-model="name" class="form-control" @keyup.enter="addRecord">
    ...
    <div class="card mt-3">
        <table>
            ...
            <tbody>
                ...
                <tr v-for="item in expensesFetch.data" :key="item.id">
                    <td class="col-9">
...
export default {
    ...
    methods: {
        ...
        async addRecord() {
            try {
                const response = await axios.post('api/expenses_account/add', {
                    name: this.name
                });
                ...
                if (response.data.status === 201) {
                    ...
                    this.addError = ('*' + response.data.message.name + '*');
        ...
        async dataFetch(page) {
            this.loading = ! false;
            try {
                const response = await axios.get(`api/expenses_account/index?page=${page}&per_page=${this.itemsPerPage}`);
                this.expensesFetch = response.data.data;
        ...
        startEditing(item) {
            item.editing = true;
            item.editedCategoryName = item.name;
        },
        ...
        updateRecord(item) {
            axios
                .post(`api/expenses_account/update/${item.id}`, {
                    name: item.editedCategoryName,
                })
                .then((response) => {
                    if (response.data.status === 200) {
                        ...
                        item.editing = false;
                        item.name = item.editedCategoryName;
                    } else {
                        console.warn('Unexpected response status:', response.data.status);
                    }
                })
        ...
        deleteRecord(Id) {
            if (confirm('Are You Sure You Want to Delete This Data')) {
                ...
                axios.delete(`api/expenses_account/delete/${Id}`)
                    .then(res => {
                        this.dataFetch();
