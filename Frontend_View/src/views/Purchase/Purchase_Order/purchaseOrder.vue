<template>
    <div class="header">
        <div class="headerinfo">
            ...
            <div class="buttn">
                <router-link to="/purchaseorderadd" type="button" class="btn1"><i class="fa-solid fa-plus"></i>Add
                    New</router-link>
            </div>
        </div>
    </div>
        ...
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                <tr v-for="item in purchaseOrderData.data" :key="item.id">
                    ...
                    <td>{{ item.po_no }}</td>
                    <td>{{ formatTableDate(item.order_date) }}</td>
                    ...
                    <td>
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <router-link :to="{ path: '/purchaseorderView/' + item.id }">
...
export default {
    ...
    methods: {
        async dataFetch(page, filterParams = '') {
            this.loading = ! false;
            try {
                const response = await axios.get(`api/purchase_order/index?${filterParams}&page=${page}&per_page=${this.itemsPerPage}`);
                ...
                console.log('Data Fetching Error Occur', error)
            }
        },
        ...
        async markAsSent(productId, currentStatus) {
            ...
            try {
                const response = await axios.get(`api/purchase_order/status/${productId}/${newStatus}`);
        ...
        deleteRecord(Id) {
            if (confirm('Are You Sure You Want to Delete This Data')) {
                ...
                axios.delete(`api/purchase_order/delete/${Id}`)
                    .then(res => {
                        this.dataFetch();
