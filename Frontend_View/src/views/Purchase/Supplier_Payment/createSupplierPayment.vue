...
<template>
    <div class="back">
        <router-link to="/supplierpayment" class="back-icon"> <i
        ...
        <form class="row" @submit.prevent="sendRecord">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select class="form-select" v-model="selectedSupplier" @change="onSupplierChange">
            ...
            <div class="col-md-3">
                <label for="input5" class="form-label">Ref No</label>
                    ...
                    <table v-if="showTable" class="table mt-3">
                        ...
                        <tbody>
                            <tr v-for="item in tableData" :key="item.id">
                                <!-- <td colspan="1"><input type="checkbox" v-model="item.isChecked" @change="updateAmount"
                                ...
                                <td colspan="2">{{ item.invoice_date }}</td>
                                <td colspan="2">{{ item.total_value }}</td>
                                <td colspan="1">{{ item.balance }}</td>
                                <td colspan="1"><input type="number" placeholder="0" v-model="item.paying"
                                        @input="updateChangedItems(item)" :id="'payingInput_' + item.id" min="0"></td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="col-12 mt-4">
                ...
                <router-link to="/supplierpayment" class="cancel"><input type="reset" value="Cancel"
    ...
    methods: {
        ...
        onSupplierChange() {
            if (this.selectedSupplier) {
                this.showTable = true;
                this.getTableRecord(this.selectedSupplier);
            }
            this.changedItems = [];
            this.amount = 0;
        },
        ...
        async getTableRecord(supplierId) {
            try {
                const response = await axios.get(`api/purchase_invoice/invoice_supplier/${supplierId}`);
        ...
        //     existingItem.paying = item.paying;
        //   } else {
        //     // Add new item to the array
        //     this.changedItems.push({ id: item.id, paying: item.paying });
        //   }
        // },
        ...
        updateChangedItems(item) {
            ...
            item.paying = Math.min(item.paying, item.balance);

            // Check if item.paying is greater than zero before adding to changedItems
            if (item.paying > 0) {
        ...
        calculateTotal() {
            ...
            this.amount = this.changedItems.reduce((sum, item) => sum + item.paying, 0);
        },
        ...
        async sendRecord() {
            ...
            try {
                ...
                const response = await axios.post('api/supplier_payment/add', formData)
                if (response.data.status == 201) {
                    this.$router.push('/supplierpayment')
                }
