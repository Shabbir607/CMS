        ...
        <div class="headerinfo">
            ...
            <div class="buttn">
                <a type="button" class="btn1" @:click="addRecordBtn">
                    ...
                    <span class="fw-bold">Add New Brand</span>
                </a>
            </div>
        </div>
    </div>

    <!-- Add New Brand Canvas -->
    <div class="offcanvas offcanvas-end" tabindex="-1" id="addNewCategory"
        aria-labelledby="offcanvasRightLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasRightLabel"> Brand / Add</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div class="col-md-12">
                <label class="form-label">Brand Name</label>
                <input type="text" v-model="brand" class="form-control" @keyup.enter="addRecord">
    ...
    <div class="card mt-3">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="2" v-if="this.loading">
                ...
                <tr v-for="item in Brandfetch.data" :key="item.id">
                    <td class="col-9">
                        <span v-if="!item.editing">{{ item.brand_name }}</span>
                        <input v-else type="text" class="editable" v-model="item.editedCategoryName"
                    ...
                    <td class="col-3">
                        ...
                        <button v-if="item.editing" class="btn btn-danger btn-sm mx-2 fw-bold" @click="cancelEditing(item)">
                            Cancel
                        </button>
...
export default {
    ...
    methods: {
        ...
        async addRecord() {
            try {
                const response = await axios.post('api/item/brand_add', {
                ...
                if (response.data.status === 201) {
                    ...
                    this.addError = ('*' + response.data.message.brand_name + '*');
        ...
        async dataFetch(page) {
            this.loading = ! false;
            try {
                const response = await axios.get(`api/item/brand?page=${page}&per_page=${this.itemsPerPage}`);
                this.Brandfetch = response.data.data;
        ...
        startEditing(item) {
            item.editing = true;
            item.editedCategoryName = item.brand_name;
        },
        ...
        updateRecord(item) {
            axios
                .post(`api/item/brand_update/${item.id}`, {
                    brand_name: item.editedCategoryName,
                })
                .then((response) => {
                    if (response.data.status === 200) {
                        ...
                        item.editing = false;
                        item.brand_name = item.editedCategoryName;
                    } else {
                        console.warn('Unexpected response status:', response.data.status);
                    }
                })
        ...
        deleteRecord(Id) {
            if (confirm('Are You Sure You Want to Delete This Data')) {
                ...
                axios.delete(`api/item/brand_delete/${Id}`)
                    .then(res => {
                        this.dataFetch();
