<template>
        ...
        <div class="headerinfo">
            ...
            <div class="buttn">
                <a type="button" class="btn1" @click="addRecordBtn">
                    ...
                    <span class="fw-bold">Add New Category</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="addNewCategory" aria-labelledby="offcanvasRightLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasRightLabel"> New Category/ Add</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div class="col-md-12">
                <label class="form-label">Category Name</label>
                <input type="text" v-model="category" @keyup.enter="addRecord" class="form-control" name="category_name">
    ...
    <div class="card mt-3">
        <table>
            ...
            <tbody>
                ...
                <tr v-for="item in Categoryfetch.data" :key="item.id">
                    <td class="col-9">
                        <span v-if="!item.editing">{{ item.category_name }}</span>
...
export default {
    ...
    methods: {
        ...
        async addRecord() {
            try {
                const response = await axios.post('api/item/category_add', {
                ...
                if (response.data.status === 201) {
                    ...
                    this.addError = ('*' + response.data.message.category_name + '*');
        ...
        async dataFetch(page) {
            this.loading = ! false;
            try {
                const response = await axios.get(`api/item/category?page=${page}&per_page=${this.itemsPerPage}`);
                this.Categoryfetch = response.data.data;
        ...
        cancelEditing(item) {
            // Reset the editing flag and discard changes
            item.editing = false;
        ...
        updateRecord(item) {
            axios
                .post(`api/item/category_update/${item.id}`, {
                ...
                .then((response) => {
                    if (response.data.status === 200) {
                        ...
                        item.editing = false;
                        item.category_name = item.editedCategoryName;
                    } else {
                        console.warn('Unexpected response status:', response.data.status);
                    }
                })
        ...
        deleteRecord(itemId) {
            if (confirm('Are You Sure You Want to Delete This Data')) {
                ...
                axios.delete(`api/item/category_delete/${itemId}`)
                    .then(res => {
                        this.dataFetch();
