<template>
        ...
        <div class="headerinfo">
            ...
            <div class="buttn">
                <a type="button" class="btn1" @:click="addRecordBtn">
                    ...
                    <span class="fw-bold">Add New Unit</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1"  id="addNewCategory"
        ...
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasRightLabel"> Unit / Add</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div class="col-md-12">
                <label class="form-label">Unit Name</label>
                <input type="text" v-model="unit" class="form-control" @keyup.enter="addRecord">
    ...
    <div class="card mt-3">
        <table>
            ...
            <tbody>
                ...
                <tr v-for="item in Unitfetch.data" :key="item.id">
                    <td class="col-9">
...
export default {
    ...
    methods: {
        ...
        async addRecord() {
            try {
                const response = await axios.post('api/item/unit_add', {
                    unit: this.unit
                });
                ...
                if (response.data.status === 201) {
                    ...
                    this.addError = ('*' + response.data.message.unit + '*');
        ...
        async dataFetch(page) {
            this.loading = ! false;
            try {
                const response = await axios.get(`api/item/unit?page=${page}&per_page=${this.itemsPerPage}`);
                this.Unitfetch = response.data.data;
        ...
        startEditing(item) {
            item.editing = true;
            item.editedCategoryName = item.unit;
        },
        ...
        updateRecord(item) {
            axios
                .post(`api/item/unit_update/${item.id}`, {
                    unit: item.editedCategoryName,
                })
                .then((response) => {
                    if (response.data.status === 200) {
                        ...
                        item.editing = false;
                        item.unit = item.editedCategoryName;
                    } else {
                        console.warn('Unexpected response status:', response.data.status);
                    }
                })
        ...
        deleteRecord(Id) {
            if (confirm('Are You Sure You Want to Delete This Data')) {
                ...
                axios.delete(`api/item/unit_delete/${Id}`)
                    .then(res => {
                        this.dataFetch();
